...
function getPath(id) {
    ...
    //adapter.log.debug('getPath: ' + JSON.stringify(path));
    //adapter.log.debug('cnt cache.values: ' + cache.values.children.length);
    return path;
}

function reloadCache() {
    cache = {values: {children: [], nodes: {}}};
    init();
    adapter.log.debug('cache reloaded!');
    return true;
}
...
function init() {
    return new Promise(resolve => adapter.getStates('*', (err, states) => {
        ...
        for (let i = 0; i < keys.length; i++) {
            let longKey = keys[i];
            let key = ownUtils.removeNameSpace(longKey);
...
function getSubStates(n) {
    ...
    //adapter.log.debug('getSubStates(n): ' + JSON.stringify(a));
    return a;
}
...
function gets(name) {
    ...
    //adapter.log.debug('gets(name): ' + JSON.stringify(r));
    return r;
}
...
function setExternal(id, state) {
    ...
    let name = ownUtils.removeNameSpace(id);
...
function delObject(id) {
    ...
    path.obj = undefined;
    //löschen aus eigenem Obj (getPath ... cache) fehlt noch!
    return adapter.delObjectAsync(id);
}

function on(str, obj, triggeredByOtherService) {
    if (ownUtils.isEmpty(triggeredByOtherService)) {
